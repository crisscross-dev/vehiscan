...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sidebarToggle) {
    sidebarToggle.addEventListener('click', (e) => {
      ...
      if (sidebarOpen) {
        ...
        if (hamburgerIcon) hamburgerIcon.classList.remove('hamburger-rotated');
        adminPanelLog('[SIDEBAR] Opened - classes:', sidebar.className);
      } else {
        sidebar.classList.remove('sidebar-open');
        sidebar.classList.add('sidebar-closed');
        if (hamburgerIcon) hamburgerIcon.classList.add('hamburger-rotated');
  ...
  function initSidebarGroupFallback() {
    ...
    sidebarGroupButtons.forEach((btn) => {
      ...
      if (icon) {
        icon.classList.toggle('rotated', isOpenByDefault);
      }
      ...
      btn.addEventListener('click', (event) => {
        ...
        if (icon) icon.classList.toggle('rotated', openState);
      });
    });
  }
  ...
  if (userChevron) userChevron.classList.add('user-chevron');
  ...
  userTrigger?.addEventListener('click', (e) => {
    ...
    e.stopPropagation();
    const isHidden = !userDropdown.classList.contains('open');

    adminPanelLog('[USER-DROPDOWN] Current state:', { isHidden, open: userDropdown.classList.contains('open') });
    ...
    if (userChevron) {
      if (isHidden) userChevron.classList.add('rotated'); else userChevron.classList.remove('rotated');
    }
  });
  ...
  document.addEventListener('click', (e) => {
    ...
    if (userDropdown?.classList.contains('open') && !clickedUserTrigger && !clickedUserDropdown) {
      closeUserDropdown();
    }
  ...
  async function loadPage(page) {
    ...
    // Fade out using CSP-safe classes instead of inline styles
    if (contentArea) contentArea.classList.add('vs-transition', 'vs-hidden');
      ...
      dashboard: `
        <div class="p-6 space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="ta-skeleton ta-skeleton-h-5rem"></div>
            <div class="ta-skeleton ta-skeleton-h-5rem"></div>
            <div class="ta-skeleton ta-skeleton-h-5rem"></div>
            <div class="ta-skeleton ta-skeleton-h-5rem"></div>
          </div>
          <div class="ta-skeleton ta-skeleton-h-18rem"></div>
        </div>`,
      _table: `
        <div class="p-6 space-y-4">
          <div class="flex gap-3 flex-wrap">
            <div class="ta-skeleton ta-skeleton-w-8rem ta-skeleton-h-2-5rem"></div>
            <div class="ta-skeleton ta-skeleton-w-8rem ta-skeleton-h-2-5rem"></div>
            <div class="ta-skeleton ta-skeleton-w-14rem ta-skeleton-h-2-5rem ta-skeleton-ml-auto"></div>
          </div>
          <div class="ta-skeleton ta-skeleton-h-2-75rem"></div>
    ...
    // Fade in loading state using class toggles
    if (contentArea) contentArea.classList.remove('vs-hidden');
    ...
    try {
      ...
      // Fade out before changing content (class-based)
      if (contentArea) contentArea.classList.add('vs-transition', 'vs-hidden');
      ...
      // Fade in new content using class toggles; remove transition helper after it finishes
      if (contentArea) contentArea.classList.remove('vs-hidden');
      setTimeout(function() { if (contentArea) contentArea.classList.remove('vs-transition'); }, 350);
      ...
      console.error(err);
      contentArea.innerHTML = "<p class='ta-text-danger'>Failed to load page</p>";
    ...
    if (applyBtn) {
      applyBtn.addEventListener('click', async function (e) {
        ...
        try {
          ...
          contentArea.innerHTML = "<p class='ta-text-danger'>Failed to apply filters</p>";
  ...
  async function openModal(url) {
    ...
    try {
      ...
      modalBody.innerHTML = '<p class="ta-text-danger">Failed to load form</p>';
    }
  }
  ...
  function updateSearchCounter(searchCount, visibleCount, totalRows, label = 'records') {
    ...
    searchCount.classList.toggle('ta-text-success', visibleCount > 0);
    searchCount.classList.toggle('ta-text-danger', visibleCount <= 0);
  }

...
<head>
    ...
    <script src="../assets/js/libs/chart.umd.min.js"></script>
        ...
        <script>
            ...
            (function(){
                try {
                    document.documentElement.setAttribute('data-homeowner-pending', '<?php echo (int)($stats['pending'] ?? 0); ?>');
                    document.documentElement.setAttribute('data-homeowner-open-requests', '<?php echo (int)$openProfileRequestsCount; ?>');
                    window.__HOMEOWNER_DEFAULT_DRAFT__ = {
...
<body class="m-0 p-0 overflow-hidden bg-gray-50 dark:bg-slate-900 transition-colors duration-300">
    ...
    <div id="imageZoomModal" class="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 hidden z-50" role="dialog" aria-modal="true" aria-label="Image zoom viewer" aria-hidden="true">
        <button type="button" class="absolute top-4 right-4 text-white text-4xl hover:text-gray-300 transition-colors z-10" data-action="closeImageZoom" aria-label="Close image zoom">&times;</button>
